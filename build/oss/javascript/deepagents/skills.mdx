...
- **Read-only skills**: Use [filesystem permissions](/oss/javascript/deepagents/permissions) to deny write operations under skill paths. See [read-only skills from a store](#read-only-skills-from-a-store) for a production pattern.
